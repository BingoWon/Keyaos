...
import {
	buildBaseRedirectToFromCurrentUrl,
	buildEmailRedirectToFromCurrentUrl,
...
export function AuthModal({ open, onOpenChange }: AuthModalProps) {
	...
	const [emailCooldownUntilMs, setEmailCooldownUntilMs] = useState<
		number | null
	>(null);
	...
	const handleForgotPassword = async (e: React.FormEvent) => {
		...
		if (emailCooldownSecondsLeft > 0) {
			setError(
				t("authModal.errors.cooldownWait", {
					seconds: emailCooldownSecondsLeft,
				}),
			);
			return;
		}
			...
			<DialogContent className="max-w-md">
				...
				<div className="pt-4">
					...
					{passwordView === "sign_in" && (
						<div className="pb-4">
							{/* Keyaos uses Clerk for authentication — no third-party OAuth here */}
							...
							>
								{loading
									...
									: emailCooldownSecondsLeft > 0
										? t("authModal.actions.waitCooldown", {
												seconds: emailCooldownSecondsLeft,
											})
