...
export class TransactionsDao {
    ...
    async createTransaction(tx: Omit<DbTransaction, "id" | "created_at">): Promise<string> {
    ...
    async getTransactionsForBuyer(buyerId: string, limit: number = 50): Promise<DbTransaction[]> {
        const res = await this.db
            .prepare("SELECT * FROM transactions WHERE buyer_id = ? ORDER BY created_at DESC LIMIT ?")
            .bind(buyerId, limit)
            .all<DbTransaction>();
